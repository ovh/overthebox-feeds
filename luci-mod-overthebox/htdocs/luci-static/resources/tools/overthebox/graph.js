...
return L.Class.extend({
	newGraph: function (name, type, viewWidth) {
            ...
            graph.wscale.name = graph.type+'_scale';
            return graph
        },
	...
	// Compute Hlabels
	computeHlabels: function (height, ratio) {
            const v = (height / ratio);
	...
	computeHpoint: function (height, hscale, point) {
        ...
        setLegends: function (svg, hlabels, wscale) {
            for (let i = 25; i < 100; i += 25) {
                svg.getElementById('label_'+i).firstChild.data = this.rate(hlabels['l'+i]).join('');
            }

            svg.parentNode.parentNode.querySelector('#'+wscale.name).firstChild.data = _('(%d minute window, %d second interval)').format(wscale.timeframe, wscale.interval);
        },
