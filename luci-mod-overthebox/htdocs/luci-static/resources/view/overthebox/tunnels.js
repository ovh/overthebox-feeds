...
return view.extend({
	...
	render: function (data) {
			...
			if (dev_i == devs.length) {
				...
				for(var tun_i = 0; tun_i < tunnels.length; tun_i++) {
					...
					upgraph[3].appendChild(CreateGraphBotton(tunnels[tun_i], color, {
						...
						bw_avg:  tunnels[tun_i]+ '_tx_bw_avg',
						bw_peak: tunnels[tun_i]+ '_tx_bw_peak',
					}));
					dngraph[3].appendChild(CreateGraphBotton(tunnels[tun_i], color, {
						bw_cur: tunnels[tun_i]+ '_rx_bw_cur',
						bw_avg: tunnels[tun_i]+ '_rx_bw_avg',
						bw_peak:tunnels[tun_i]+ '_rx_bw_peak',
					}));

					lines[0].push({line : 'tx_' + tunnels[tun_i], counter: true});
					lines[0].push(null);

					lines[1].push({line : 'rx_' + tunnels[tun_i], counter: true});
					lines[1].push(null);
				}

				body.lastElementChild.appendChild(E('div', { 'class' : 'selection', 'data-tab': 'all', 'data-tab-title': 'all' },[
					E('div', {'class':'title'}, _('Upload')),
					E(upgraph),
					E('div', {'class':'title'}, _('Download')),
					E(dngraph),
				]));

				function updategraphCB(svg, info) {
					...
					if (scale == 'scale') {
						...
						for (var tun_i = 0; tun_i < tunnels.length; tun_i++) {
							dom.content(tab.querySelector('#' + tunnels[tun_i] + '_' + direction + '_bw_cur'), rate(info.line_current[tun_i], true));
							dom.content(tab.querySelector('#' + tunnels[tun_i] + '_' + direction + '_bw_avg'), rate(info.line_average[tun_i], true));
							dom.content(tab.querySelector('#' + tunnels[tun_i] + '_' + direction + '_bw_peak'), rate(info.line_peak[tun_i], true));
						}
					}
				}

				if (tunnels.length) {
					/* Set 1 parameter to mark that graph display multi interfaces*/
					this.updateGraph(tunnels[0], csvgs[0], lines[0], {up: 1, dn: 0, num: tunnels.length}, updategraphCB);
					this.updateGraph(tunnels[0], csvgs[1], lines[1], {up: 0, dn: 1, num: tunnels.length}, updategraphCB);
				}
			}
			else {
				if(!devs[dev_i].dev.flags.pointtopoint)
					continue;
		...
		ui.tabs.initTabGroup(body.lastElementChild.childNodes);
