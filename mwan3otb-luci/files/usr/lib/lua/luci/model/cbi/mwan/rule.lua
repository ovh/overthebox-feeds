...
function ruleCheck() -- determine if rules needs a proper protocol configured
	uci.cursor():foreach("mwan3", "rule",
		function (section)
			local sourcePort = ut.trim(sys.exec("uci -q -p /var/state get mwan3." .. section[".name"] .. ".src_port"))
			local destPort = ut.trim(sys.exec("uci -q -p /var/state get mwan3." .. section[".name"] .. ".dest_port"))
			if sourcePort ~= "" or destPort ~= "" then -- ports configured
				local protocol = ut.trim(sys.exec("uci -q -p /var/state get mwan3." .. section[".name"] .. ".proto"))
