...
function interfaceCheck()
	metricValue = ut.trim(sys.exec("uci -q -p /var/state get network." .. arg[1] .. ".metric"))
	if metricValue == "" then -- no metric
		...
		uci.cursor():foreach("mwan3", "interface",
			function (section)
				local metricValue = ut.trim(sys.exec("uci -q -p /var/state get network." .. section[".name"] .. ".metric"))
	...
	local trackingNumber = tonumber(ut.trim(sys.exec("echo $(uci -q -p /var/state get mwan3." .. arg[1] .. ".track_ip) | wc -w")))
	if trackingNumber > 0 then
		local reliabilityNumber = tonumber(ut.trim(sys.exec("uci -q -p /var/state get mwan3." .. arg[1] .. ".reliability")))
	...
	if ut.trim(sys.exec("uci -q -p /var/state get network." .. arg[1])) == "interface" then
		local interfaceDevice = ut.trim(sys.exec("uci -q -p /var/state get network." .. arg[1] .. ".ifname"))
...
gen = mwan_interface:option(DummyValue, "generated", translate("Generated tracker"),
        translate("This is an autogenerated interface tracker"))
gen:depends("generated", "1")

edite = mwan_interface:option(Flag, "edited", translate("Keep settings"),
