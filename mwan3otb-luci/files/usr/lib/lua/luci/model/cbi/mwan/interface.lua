...
function interfaceCheck() -- find issues with too many interfaces, reliability and metric
	uci.cursor():foreach("mwan3", "interface",
		function (section)
			...
			local metricValue = ut.trim(sys.exec("uci -q -p /var/state get network." .. interfaceName .. ".metric"))
			if metricValue == "" then
				errorFound = 1
			...
			local trackingNumber = tonumber(ut.trim(sys.exec("echo $(uci -q -p /var/state get mwan3." .. interfaceName .. ".track_ip) | wc -w")))
			if trackingNumber > 0 then
				local reliabilityNumber = tonumber(ut.trim(sys.exec("uci -q -p /var/state get mwan3." .. interfaceName .. ".reliability")))
			...
			if ut.trim(sys.exec("uci -q -p /var/state get network." .. interfaceName)) == "interface" then
				local interfaceDevice = ut.trim(sys.exec("uci -q -p /var/state get network." .. interfaceName .. ".ifname"))
...
metric = mwan_interface:option(DummyValue, "metric", translate("Metric"))
	...
	function metric.cfgvalue(self, s)
		local metricValue = sys.exec("uci -q -p /var/state get network." .. s .. ".metric")
		if metricValue ~= "" then
			return metricValue
		else
			return "&#8212;"
